...
#define	CONFIG_EXTRA_ENV_SETTINGS					\
		...
		"ethprime=FEC0\0"					\
		"uboot=u-boot.bin\0"			\
		"KernelNANDAddress=0x000000\0" \
		"KernelNANDSize=0xA00000\0" \
		"kernel=uImage\0"				\
		"nfsroot=/nfs/target\0"				\
		"video_args=mxcfb0:dev=hdmi,1280x720M@60,if=RGB24\0"	\
		"bootargs_base=setenv bootargs console=ttymxc0,115200 video=${video_args} \0"\
		"bootargs_nfs=setenv bootargs ${bootargs} root=/dev/nfs "\
			"ip=dhcp nfsroot=${serverip}:${nfsroot},v3,tcp\0"\
		"bootcmd_net=run bootargs_base bootargs_nfs; "		\
			"tftpboot ${loadaddr} ${kernel}; bootm\0"	\
		"bootargs_mmc=setenv bootargs ${bootargs} "     \
			"root=/dev/mmcblk0p2 rootwait ip=none\0"         \
		"bootcmd_mmc=run bootargs_base bootargs_mmc; "   \
		"mmc rescan; "	\
		"fatload mmc 0:1 ${loadaddr} uImage; bootm\0"	\
		"bootcmd=run bootcmd_mmc\0"                             \
		"bootcmd_nand=run bootargs_mmc; "		\
			"nand read ${loadaddr} ${KernelNANDAddress} ${KernelNANDSize}; bootm\0"	
...
#define CONFIG_SYS_PROMPT		"BCT-RE3 U-Boot > "
...
#define CONFIG_CMD_DHCP
#define CONFIG_CMD_PING
#define CONFIG_IPADDR			10.0.0.240
#define CONFIG_SERVERIP			10.0.0.121
